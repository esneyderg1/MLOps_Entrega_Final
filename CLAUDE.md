...
- [x] **(2026-09-22)** `uv run ruff check .` y `uv run pytest` en verde sobre todo el repo.
  *Verificado:* 45/45 tests, `ruff check` ("All checks passed!") y `ruff format --check` (40 archivos formateados) en verde. Re-verificado el 2026-09-24 tras las mejoras de la interfaz web: **46/46 tests** (el nuevo cubre `OTHER` en los campos de país), `ruff check` limpio y 41 archivos formateados.
...
- [ ] Preparar la presentación del proyecto.
  *Avance (2026-09-24):* slides armadas en `docs/presentacion.html` — 7 slides que abren sin internet (portada · problema/dataset/EDA · procesamiento sin leakage + Optuna · escalera de resultados · comparación de las familias de modelos · registro + orquestación en vivo · demo de cierre) y reparto de temas acordado en `docs/presentacion.md`. **Falta para marcarla:** revisión de las slides por el equipo y ensayo de la corrida en vivo (`pipeline_flow` y `docker compose up --build`, que debe reconstruir la imagen para que el contenedor traiga la interfaz nueva). Hasta entonces sigue en `[ ]` (regla de oro).
